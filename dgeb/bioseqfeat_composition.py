...
# AAC -> (20,), DPC -> (400,), PseAAC(lambda_=30) -> (50,), total = 470
_LAMBDA = 30
_FEAT_DIM = 20 + 400 + (20 + _LAMBDA)
...
class BioSeqFeatCompositionTransformer(BioSeqTransformer):
    """DGEB-compatible model that produces embeddings from composition features.

    Uses AAC (20-d) + DPC (400-d) + PseAAC(lambda_=30) (50-d)
    concatenated into a 470-d vector. No HuggingFace model is loaded.

    Example
    -------
    >>> model = BioSeqFeatCompositionTransformer()
    >>> evaluation = dgeb.DGEB(tasks=dgeb.get_tasks_by_modality(dgeb.Modality.PROTEIN))
    >>> results = evaluation.run(model, output_folder="results/bioseqfeat_composition")
    """

    MODEL_NAMES = ["bioseqfeat-composition"]

    def __init__(self, **kwargs):
        self._pipeline = Pipeline(
            featurizers=[AAC(), DPC(), PseAAC(lambda_=_LAMBDA)], weights=[1.0, 1.0, 8.0]
        )
        self._tokenizer_instance = _BioSeqFeatTokenizer()
        kwargs.setdefault("num_processes", 0)
        super().__init__(model_name="bioseqfeat-composition", **kwargs)
